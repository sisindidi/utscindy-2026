    ...
    <div class="col-12">
        ...
        @if($dailyCashProject && $dailyCashProject->detail)
            <div class="main-laporan-section" style="background: #fff; border-radius: 15px; padding: 35px; box-shadow: 0 4px 20px rgba(138, 63, 252, 0.05); border: 1px solid #e8dbff; font-family: 'Poppins', sans-serif;">
                ...
                <h4 style="color: #6929c4; font-weight: bold; margin-bottom: 25px; font-size: 28px; border-bottom: 2px solid #f1f1f1; padding-bottom: 15px;">
                    📋 Detail Project Akhir
                </h4>

                @if($dailyCashProject->detail->project_type)
                    <p style="font-size: 25px;"><strong>Jenis Pengujian:</strong> <span class="badge" style="background: #8a3ffc; color: #fff; padding: 5px 12px; border-radius: 5px;">{{ $dailyCashProject->detail->project_type }}</span></p>
                @endif

                @if($dailyCashProject->detail->background)
                    <h5 style="font-weight: bold; color: #8a3ffc; margin-top: 30px; border-left: 4px solid #8a3ffc; padding-left: 12px; font-size: 20px;">A. Latar Belakang</h5>
                ...
                @if($dailyCashProject->detail->problem_analysis || $dailyCashProject->detail->system_requirements)
                    <h5 style="font-weight: bold; color: #8a3ffc; margin-top: 30px; border-left: 4px solid #8a3ffc; padding-left: 12px; font-size: 18px;">B. Analisis & Kebutuhan Sistem</h5>
                    <div class="pl-3 mt-2">
                        @if($dailyCashProject->detail->problem_analysis)
                            <strong style="font-size: 18px; color: #333;" class="d-block">Analisis Masalah:</strong>
                            <div style="font-size: 18px; color: #555; line-height: 1.7;">{!! $dailyCashProject->detail->problem_analysis !!}</div>
                        @endif
                        @if($dailyCashProject->detail->system_requirements)
                            <strong style="font-size: 18px; color: #333;" class="mt-3 d-block">Kebutuhan Fitur:</strong>
                            <div style="font-size: 18px; color: #555; line-height: 1.7;">{!! $dailyCashProject->detail->system_requirements !!}</div>
                        @endif
                    </div>
                @endif

                <h5 style="font-weight: bold; color: #8a3ffc; margin-top: 30px; border-left: 4px solid #8a3ffc; padding-left: 12px; font-size: 20px;">C. Spesifikasi Teknologi</h5>
                ...
                @if($dailyCashProject->detail->workflow_commands)
                    <strong style="font-size: 16px; color: #333;" class="mt-3 d-block pl-3">Workflow Perintah Terminal:</strong>
                ...
                @if($dailyCashProject->detail->erd_image)
                    <h5 style="font-weight: bold; color: #8a3ffc; margin-top: 30px; border-left: 4px solid #8a3ffc; padding-left: 12px; font-size: 20px;">D. Rancangan ERD</h5>
